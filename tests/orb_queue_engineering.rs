//! Orb-queue engineering regressions for target-, order-, and timing-aware
//! Dark-orb planning.
...
fn holds_a_young_dark_orb_instead_of_popping_it_into_a_boss() {
    ...
    game.player.hand = vec![Card::new(CardId::Dualcast)].into();
...
fn does_not_overflow_evict_the_dark_bank_with_zap() {
    ...
    game.player.energy = 1;
    game.player.hand = vec![Card::new(CardId::Zap)].into();
...
fn declines_glacier_when_its_second_frost_would_evict_the_dark_bank() {
    ...
    game.player.hand = vec![Card::new(CardId::Glacier)].into();
    game.player.max_orbs = 3;
    game.player.orbs = vec![
        Orb {
            kind: OrbKind::Dark,
...
fn steers_the_evoke_by_shaping_hp_before_releasing_the_bank() {
    ...
    game.player.energy = 3;
    game.player.hand = vec![Card::new(CardId::Zap), Card::new(CardId::Strike_B)].into();
    game.player.max_orbs = 3;
    game.player.orbs = vec![
        Orb {
            kind: OrbKind::Dark,
